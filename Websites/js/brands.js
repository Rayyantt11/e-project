document.addEventListener("DOMContentLoaded", async () => {
    try {
        ...
        const data = await response.json(); // Now directly the array, no need for data.cars

        data.forEach(car => { // Use 'data' directly
                        ...
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content bg-dark text-light">
                                <div class="modal-header">
                                    <h1 class="modal-title fs-5 bg-light" id="${modalId}-label">${car.title}</h1>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <img src="${car.pic}" class="car-image mb-3" loading="lazy" alt="${car.title}">
                                    <span class="mb-2">
